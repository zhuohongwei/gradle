...
public class JacocoPlugin implements Plugin<ProjectInternal> {
    ...
    public Object configureDefaultOutputPathForJacocoMerge() {
        ...
        project.getTasks().withType(JacocoMerge.class).configureEach(task -> task.setDestinationFile(project.provider(() -> new File(project.getBuildDir(), "/jacoco/" + task.getName() + ".exec"))));
    ...
    private Action<ConfigurableReport> configureReportOutputDirectory(final JacocoPluginExtension extension, final JacocoReport reportTask) {
        return report -> {
            if (report.getOutputType().equals(Report.OutputType.DIRECTORY)) {
                report.setDestination(project.provider(() -> new File(extension.getReportsDir(), reportTask.getName() + "/" + report.getName())));
            } else {
                report.setDestination(project.provider(() -> new File(extension.getReportsDir(), reportTask.getName() + "/" + reportTask.getName() + "." + report.getName())));
            }
        };
    }

    private void configureJacocoReportDefaults(final JacocoPluginExtension extension, final JacocoReport reportTask) {
        reportTask.getReports().all(report -> report.setEnabled(project.provider(() -> report.getName().equals("html"))));
        ...
        project.getTasks().register(
            ...
            reportTask -> {
                ...
                reportTask.getReports().all(report -> {
                     ...
                     */
                    if (report.getOutputType().equals(Report.OutputType.DIRECTORY)) {
                        report.setDestination(project.provider(() -> new File(extension.getReportsDir(), testTaskProvider.getName() + "/" + report.getName())));
                    } else {
                        report.setDestination(project.provider(() -> new File(extension.getReportsDir(), testTaskProvider.getName() + "/" + reportTask.getName() + "." + report.getName())));
                    }
                });
            });
    }
