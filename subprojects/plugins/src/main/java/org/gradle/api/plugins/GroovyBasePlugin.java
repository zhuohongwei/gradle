...
public class GroovyBasePlugin implements Plugin<Project> {
    ...
    private void configureSourceSetDefaults() {
        project.getConvention().getPlugin(JavaPluginConvention.class).getSourceSets().all(sourceSet -> {
            ...
            sourceSet.getResources().getFilter().exclude(element -> groovySourceSet.getGroovy().contains(element.getFile()));
            sourceSet.getAllJava().source(groovySourceSet.getGroovy());
            sourceSet.getAllSource().source(groovySourceSet.getGroovy());

            final Provider<GroovyCompile> compileTask = project.getTasks().register(sourceSet.getCompileTaskName("groovy"), GroovyCompile.class, compile -> {
