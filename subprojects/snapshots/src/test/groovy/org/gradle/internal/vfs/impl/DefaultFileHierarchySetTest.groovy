...
class DefaultFileHierarchySetTest extends Specification {
    ...
    def "can add dir to empty set"() {
        ...
        expect:
        def s1 = empty.update(snapshot.getAbsolutePath(), dir1Snapshot)
        snapshotPresent(s1, dir1)
        !snapshotPresent(s1, dir2)

        def s2 = empty.update(snapshot.getAbsolutePath(), dir2Snapshot)
    ...
    def "can add dir to singleton set"() {
        ...
        expect:
        def s1 = single.update(snapshot.getAbsolutePath(), snapshotDir(dir2))
        snapshotPresent(s1, dir1)
        ...
        def s2 = single.update(snapshot.getAbsolutePath(), snapshotDir(dir1))
        snapshotPresent(s2, dir1)
        ...
        def s3 = single.update(snapshot.getAbsolutePath(), snapshotDir(child))
        ...
        def s4 = single.update(snapshot.getAbsolutePath(), snapshotDir(parent))
        ...
        def s5 = single.update(snapshot.getAbsolutePath(), snapshotDir(tooFew))
        ...
        def s6 = single.update(snapshot.getAbsolutePath(), snapshotDir(tooMany))
    ...
    def "can add dir to multi set"() {
        ...
        expect:
        def s1 = multi.update(snapshot.getAbsolutePath(), snapshotDir(dir3))
        snapshotPresent(s1, dir1)
        snapshotPresent(s1, child)
        snapshotPresent(s1, dir2)
        snapshotPresent(s1, dir3)
        ...
        def s2 = multi.update(snapshot.getAbsolutePath(), snapshotDir(dir2))
        snapshotPresent(s2, dir1)
        snapshotPresent(s2, child)
        snapshotPresent(s2, dir2)
        !snapshotPresent(s2, dir3)
        ...
        def s3 = multi.update(snapshot.getAbsolutePath(), snapshotDir(child))
        snapshotPresent(s3, dir1)
        snapshotPresent(s3, child)
        snapshotPresent(s3, dir2)
        !snapshotPresent(s3, dir3)
        ...
        def s4 = multi.update(snapshot.getAbsolutePath(), snapshotDir(parent))
        snapshotPresent(s4, dir1)
        snapshotPresent(s4, child)
        snapshotPresent(s4, dir2)
    ...
    def "splits and merges prefixes as directories are added"() {
        ...
        def s2 = s1.update(snapshot.getAbsolutePath(), snapshotDir(dir1dir2dir4))
        s2.flatten() == [dir1.path, "1:dir2", "2:dir3", "2:dir4", "1:dir5/and/more"]

        def s3 = s2.update(snapshot.getAbsolutePath(), snapshotDir(dir6))
        s3.flatten() == [parent.path, "1:dir1", "2:dir2", "3:dir3", "3:dir4", "2:dir5/and/more", "1:dir6"]

        def s4 = s3.update(snapshot.getAbsolutePath(), snapshotDir(dir1dir2))
        s4.flatten() == [parent.path, "1:dir1", "2:dir2", "2:dir5/and/more", "1:dir6"]

        def s5 = s4.update(snapshot.getAbsolutePath(), snapshotDir(dir1))
        s5.flatten() == [parent.path, "1:dir1", "1:dir6"]

        def s6 = s3.update(snapshot.getAbsolutePath(), snapshotDir(dir1))
        s6.flatten() == [parent.path, "1:dir1", "1:dir6"]

        def s7 = s3.update(snapshot.getAbsolutePath(), snapshotDir(parent))
    ...
    def "can update existing snapshots"() {
        ...
        when:
        child.text = "updated"
        set = invalidate(set, child).update(snapshot.getAbsolutePath(), snapshotDir(dir))
        then:
        assertDirectorySnapshot(set, dir)
        assertFileSnapshot(set, child)
    }
    ...
    def "can update file snapshot with sub-dir snapshot"() {
        ...
        dir.delete()
        child.text = "created"
        set = invalidate(set, child).update(snapshot.getAbsolutePath(), snapshotDir(dir.file("sub")))
        then:
    ...
    def "adding a snapshot in a known directory is ignored"() {
        ...
        def set = setWithDir1.update(snapshot.getAbsolutePath(), snapshotDir(subDir))
        then:
    ...
    def "root is handled correctly"() {
        ...
        when:
        def set = FileHierarchySet.EMPTY.update(snapshot.getAbsolutePath(), new DirectorySnapshot("/", "", [new RegularFileSnapshot("/root.txt", "root.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(1111)))
        then:
        set.getMetadata("/root.txt").get().type == FileType.RegularFile

        when:
        set = set.invalidate("/root.txt").update(snapshot.getAbsolutePath(), new DirectorySnapshot("/", "", [new RegularFileSnapshot("/base.txt", "base.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(2222)))
        then:
    ...
    def "can add to completely different paths"() {
        ...
        def set = FileHierarchySet.EMPTY.update(snapshot.getAbsolutePath(), new DirectorySnapshot(firstPath, "log", [new RegularFileSnapshot("${firstPath}${File.separator}root.txt", "root.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(1111)))
        .update(snapshot.getAbsolutePath(), new DirectorySnapshot(secondPath, "bin", [new RegularFileSnapshot("${secondPath}${File.separator}root.txt", "root.txt", HashCode.fromInt(1234), new FileMetadata(1, 1))], HashCode.fromInt(1111)))
    ...
    private FileHierarchySet fileHierarchySet(File location) {
        FileHierarchySet.EMPTY.update(snapshot.getAbsolutePath(), location.directory ? snapshotDir(location) : snapshotFile(location))
    }
    ...
    private FileHierarchySet fileHierarchySet(Iterable<? extends File> locations) {
        ...
        for (File location : locations) {
            set = set.update(snapshot.getAbsolutePath(), location.directory ? snapshotDir(location) : snapshotFile(location))
        }
        return set
    }
}
